...
const ALL_ACTIVE: BagActive = { handlebar: true, frame: true, seatpack: true, fork: true };

describe('computeBagRecommendation', () => {
  it('paired fork capacity is doubled internally', () => {
    const rec = computeBagRecommendation(
      0,
      { handlebar: 10, frame: 5, seatpack: 15, fork: 3 }, // fork = 3L per bag = 6L pair
      ALL_ACTIVE,
      'cumulative',
    );
    expect(rec.fork.capacityPerBagL).toBe(3);
    expect(rec.fork.capacityL).toBe(6);
    expect(rec.totalCapacity).toBe(10 + 5 + 15 + 6);
  });

  it('cumulative mode distributes total proportionally to effective capacities', () => {
    const rec = computeBagRecommendation(
      24,
      { handlebar: 10, frame: 5, seatpack: 15, fork: 3 }, // effective: 10, 5, 15, 6 → total 36
      ALL_ACTIVE,
      'cumulative',
    );
  ...
  it('inactive slot contributes zero capacity and receives zero volume', () => {
    const rec = computeBagRecommendation(
      20,
      { handlebar: 10, frame: 5, seatpack: 15, fork: 3 },
      { handlebar: true, frame: false, seatpack: true, fork: false },
      'cumulative',
    );
  ...
  it('inactive slot in "each" mode also gets zero', () => {
    const rec = computeBagRecommendation(
      10,
      DEFAULT_BAG_CAPACITIES,
      { handlebar: true, frame: true, seatpack: true, fork: false },
      'each',
    );
  ...
  it('DEFAULT_BAG_ACTIVE turns fork off by default', () => {
